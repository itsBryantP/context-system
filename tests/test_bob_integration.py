...
def test_pack_install_with_bob_creates_skill(tmp_path):
    """End-to-end: `ctx pack --install --tool bob` must wire the packed module
    into .bob/skills/<name>/ via SKILL.md generated by write_module."""
    from ctx.pack import pack

    input_dir = tmp_path / "docs"
    input_dir.mkdir()
    (input_dir / "guide.md").write_text("# Guide\n\nHow to use the thing.\n")
    (input_dir / "ref.md").write_text("# Reference\n\nAPI details.\n")

    project = tmp_path / "project"
    project.mkdir()

    pack(input_dir, install=True, tools=["bob"], project_root=project)

    skill_link = project / ".bob" / "skills" / "docs"
    assert skill_link.is_symlink(), (
        f".bob/skills/docs should be a symlink; got children: "
        f"{list((project / '.bob').rglob('*')) if (project / '.bob').exists() else 'no .bob/'}"
    )
